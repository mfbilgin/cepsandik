...
export const CreateCommunityScreen = () => {
    ...
    const handleCreate = async () => {
        if (!name.trim() || !description.trim()) {
            Toast.show({ type: 'error', text1: 'Eksik Bilgi', text2: 'Lütfen topluluk adı ve açıklamasını doldurun.' });
            return;
        }
        ...
        try {
            ...
            Toast.show({ type: 'success', text1: 'Başarılı', text2: 'Topluluk başarıyla oluşturuldu.' });
            setTimeout(() => {
                navigation.goBack();
            }, 1000);
        } catch (error: any) {
        ...
        >
            <View style={tw`w-full bg-white flex-1 flex-col relative`}>
                ...
                <View style={tw`flex-row items-center px-4 pt-14 pb-2 bg-white border-b border-slate-100 z-10`}>
                    ...
                    <Text style={tw`text-lg font-bold flex-1 text-center pr-8`}>Topluluk Oluştur</Text>
                </View>
                ...
                >
                    ...
                    <View style={tw`flex-col items-center mb-8`}>
                        <TouchableOpacity style={tw`relative`} activeOpacity={0.8} onPress={() => Toast.show({ type: 'info', text1: 'Yakında', text2: 'Logo yükleme özelliği yakında...' })}>
                        ...
                        <Text style={tw`mt-3 text-sm font-medium text-[#1162d4]`}>Logo Yükle</Text>
                    </View>
                    ...
                    <View style={tw`flex-col gap-6`}>
                        ...
                        <View style={tw`flex-col gap-1.5`}>
                            <View style={tw`flex-row justify-between items-baseline`}>
                                <Text style={tw`text-sm font-semibold text-slate-700`}>Açıklama</Text>
                            ...
                            <View style={tw`p-4 rounded-xl border border-slate-200 bg-white shadow-sm`}>
                                <View style={tw`flex-row items-center justify-between mb-2`}>
                                    <View style={tw`flex-row items-center gap-3`}>
                                        ...
                                        <Text style={tw`font-semibold text-slate-900`}>Gizli Topluluk</Text>
                                    </View>
                                    <Switch
                                ...
                                <Text style={tw`text-sm text-slate-500 pl-[52px] leading-relaxed`}>
                                    Gizli topluluklar sadece davet ile girilebilir ve arama sonuçlarında görünmezler.
                                </Text>
                            </View>
                        </View>
                    </View>
                ...
                <View style={tw`absolute bottom-0 left-0 w-full p-4 bg-white border-t border-slate-100 pb-8`}>
                    <TouchableOpacity
                        ...
                        activeOpacity={0.8}
                    >
                        <Text style={tw`text-white font-bold text-base`}>{isLoading ? 'Oluşturuluyor...' : 'Topluluğu Kur'}</Text>
